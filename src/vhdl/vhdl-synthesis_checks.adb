      ...
      then
         ...
         while Stmt /= Null_Iir loop
            ...
            if Kind = Iir_Kind_Sensitized_Process_Statement then
               ...
               -- and also skip processes that originate from places different
               -- than the user sources (these are automatically generated by
               -- the canonization process with correct sensitivity lists, and
               -- also they don't have named entities so Get_Named_Entity()
               -- fails for them)
